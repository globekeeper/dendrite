	...
	for _, event := range addedEvents {
		...
		if strings.HasPrefix(event.Type(), "connect.data_retention") {
			...
			for _, spaceId := range spaces {
				if spaceId == event.RoomID().String() {
					exists = true
				}
			}
			if exists {
				// Remove existing job.
				err = scheduler.RemoveByTag(event.RoomID().String())
				if err != nil {
					logrus.WithError(err).WithField("event_id", event.EventID()).Error("failed to remove data retention job by space id tag")
				}
			}
			// Create the new job.
			_, err = scheduler.At(DRContent.At).Every(DRContent.Timeframe).Milliseconds().Tag(event.RoomID().String()).Do(func(SpaceID string) {
...
func (d *Database) UpsertDataRetention(ctx context.Context, event *rstypes.HeaderedEvent) error {
	...
	if drEv.Timeframe == 0 {
		...
		err = d.ConnnectQ.DeleteDataRetention(ctx, event.RoomID().String())
		if err != nil {
