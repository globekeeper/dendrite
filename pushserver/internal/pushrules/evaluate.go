	...
	for _, defRules := range []bool{false, true} {
		for _, rsat := range rse.ruleSet {
			for _, rule := range rsat.Rules {
				...
				ok, err := rse.eventMatches(rule, rsat.Kind, event)
				if err != nil {
					return nil, err
				}
				if ok {
...
func (rse *RuleSetEvaluator) eventMatches(rule *Rule, kind Kind, event *gomatrixserverlib.Event) (bool, error) {
	if !rule.Enabled {
		return false, nil
	}
	...
	case OverrideKind, UnderrideKind:
		for _, cond := range rule.Conditions {
			ok, err := rse.conditionMatches(cond, event)
			if err != nil {
				return false, err
			}
			if !ok {
	...
	case ContentKind:
		...
		return rse.patternMatches("content.body", rule.Pattern, event)
...
func (rse *RuleSetEvaluator) conditionMatches(cond *Condition, event *gomatrixserverlib.Event) (bool, error) {
	switch cond.Kind {
	case EventMatchCondition:
		return rse.patternMatches(cond.Key, cond.Pattern, event)

	case ContainsDisplayNameCondition:
		return rse.patternMatches("content.body", rse.ec.UserDisplayName(), event)
	...
	case SenderNotificationPermissionCondition:
		return rse.ec.HasPowerLevel(event.Sender(), cond.Key)
...
func (rse *RuleSetEvaluator) patternMatches(key, pattern string, event *gomatrixserverlib.Event) (bool, error) {
